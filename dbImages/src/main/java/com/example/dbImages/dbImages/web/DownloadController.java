...
public class DownloadController {
    ...
    public HttpEntity<byte[]> download(@PathVariable("fileId") long fileId){
        ...
        FileDownloadModel fileDownloadModel = fileService.getFileById(fileId).orElseThrow();
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType(MimeTypeUtils.parseMimeType(fileDownloadModel.getContentType())));
        headers.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filName=" + fileDownloadModel.getFileName() );
